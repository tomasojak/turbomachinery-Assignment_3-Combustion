...
2. Open a Miniforge command prompt. You should be able to find it in your start menu or using the search bar.
...
4. Activate the new environment by running `mamba activate py313`.
5. Install the necessary packages into the environment by running `mamba install package-name`. For this code, you will need `numpy`, `cantera` and `matplotlib`.
6. Install an IDE (Integrated Development Environment). You can install it just like any other package. Here, Spyder is recommended (`mamba install spyder`).
7. Close the Miniforge command prompt.
8. Open Spyder or any other IDE you installed. You should be able to find it using the windows search bar.
